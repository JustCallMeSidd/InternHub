...
This project is developed with the following assumptions and constraints:

* The uploaded resume is **machine-readable** (PDF or TXT format) and not a scanned image.
* Internship descriptions are **manually provided** by the user and are assumed to be relevant and complete.
* The ATS score generated by the system is **advisory in nature** and should be used as a confidence indicator, not as a final hiring decision.
* The matching accuracy depends on the **LLM’s reasoning capability**, which may vary across different models.
* API performance may be affected by **rate limits or temporary outages**, especially when using free or shared-tier LLM services.
* The system does not store resumes or job descriptions permanently, ensuring **data privacy and security**.

These assumptions help keep the application lightweight, secure, and suitable for academic and prototype-level deployment.
